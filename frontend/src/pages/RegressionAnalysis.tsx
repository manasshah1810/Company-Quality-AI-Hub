...
export default function RegressionAnalysis() {
  ...
  // Execution timer (Accelerated 60x for simulation)
  useEffect(() => {
  ...
  // Update Progress and Stats based on Elapsed Time
  useEffect(() => {
    if (executionState === "running") {
      const progress = Math.min(100, (elapsedTime / estimatedTotalTime) * 100);
      setExecutionProgress(progress);

      const newPassed = Math.floor((progress / 100) * totalTests);
      const newFailed = Math.floor(newPassed * 0.04);
      ...
      // Update current test name every few simulated minutes
      if (Math.floor(elapsedTime / 120) % 5 === 0) {
        const testNames = [
        ...
        setCurrentTestName(testNames[Math.floor((elapsedTime / 600) % testNames.length)]);
      }
    }
  }, [elapsedTime, estimatedTotalTime, totalTests, executionState]);
                      ...
                      {analysisData.selectedSuites.map((suite, idx) => (
                        <tr key={idx} className="border-b border-background/50 hover:bg-background/20 transition">
                          ...
                          <td className="py-3 px-3">
                            <span
                              className={`px-2 py-1 rounded-full text-xs font-medium ${suite.risk_level === "HIGH"
        ...
        <div className="glass rounded-xl p-4 space-y-4">
          ...
          <div className="space-y-2 max-h-[600px] overflow-y-auto">
            {jobHistory.map((job) => (
              <div
                key={job.id}
                className={`p-3 rounded-lg cursor-pointer transition-all border ${job.status === "success"
